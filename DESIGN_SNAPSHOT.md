...
Phase 8 で導入された初回アクセス時の認証機能の確定仕様。
...
|---|---|
| 認証スコープ | 初回アクセス時のみ |
| 認証済みフラグ保管 | LocalStorage `kirari_authenticated`（無期限） |
| パスワード保管 | GAS スクリプトプロパティ `AUTH_PASSWORD` |
| 認証画面デザイン | 紺色背景（#1a237e）+ 白ロゴ + 白いカード |
| ロゴサイズ | logo.png（つながるーと）96px / logo_ai.png（AI送迎）32px |
| 失敗時挙動 | 「パスワードが違います」エラー、再試行可 |
| バイパス | なし（完全に閉じる） |
| 通信方式 | Content-Type: text/plain（CORS preflight 回避） |
...
│
▼
LocalStorage `kirari_authenticated` の値を確認
│
├── 'true' あり → 認証スキップ → メイン画面表示
│
└── なし → パスワード入力画面表示
    │
    ▼
    ...
    │
    ▼
    GAS authCheck エンドポイント呼び出し (Content-Type: text/plain)
    │
    ├── 成功 → LocalStorage に 'true' 保存 → メイン画面
    │
    └── 失敗 → エラー表示 → 再入力
```
...
| 状態 | 復帰挙動 |
|---|---|
| 認証済み + -888 あり | パスワードスキップ → 確定ルート復帰（ケースA） |
| 認証済み + -888 なし + -999 のみ | パスワードスキップ → 半自動復帰（ケースB） |
| 認証クリア（LocalStorage 削除等） | パスワード入力画面表示 |
...
### Phase 6（2026-05-14）リロード仕様の文書化

- **コミット**: `1286caa`
- **内容**: DESIGN_SNAPSHOT.md にリロード復帰仕様（3-(c)の実測動作仕様表）と修正履歴セクションを追加
- **経緯**: バグ3（リロード復帰）の解消確認後、実測動作を仕様として正式に文書化

### Phase 7-2（2026-05-14）未使用画像の削除

- **コミット**: `29b4ea6`
- **内容**: リポジトリから未使用画像ファイル（合計3.6MB）を削除
- **経緯**: リポジトリのクリーンアップ。動作には影響しないが、不要なファイルを整理

### Phase 8（2026-05-14）パスワード認証の実装

- **コミット**: `996783d`（GAS authCheck エンドポイント追加）→ `b8ffbcc`（フロント認証画面追加）→ `b835f4d`（CORS preflight 対応・text/plain 化）→ デザイン調整
- **内容**: 初回アクセス時のパスワード認証画面を実装
- **詳細**:
  - GAS スクリプトプロパティ `AUTH_PASSWORD` で認証パスワードを管理
  - LocalStorage `kirari_authenticated` で認証済みフラグを永続化（無期限）
  - Content-Type: text/plain で CORS preflight を回避
- **経緯**: コンテストデモとして閲覧制限を設けるため、最小限の認証を導入

### Phase 9（2026-05-15）GAS departTime Date型変換修正

- **コミット**: `3d39141`
- **内容**: GAS_script.gs の doGet で、`departTime` が Date オブジェクトの場合に `Utilities.formatDate(row[2], tz, 'HH:mm')` で文字列に変換するよう修正
- **経緯**: Google Sheets が `"09:00"` のような HH:MM 文字列を内部的に Date 型に自動変換するため、doGet で取得すると `"Fri Dec 30 1899 09:00:00 GMT+0900"` のような文字列になり、フロントの正規表現で弾かれて時刻表示が消える不具合があった

### Phase 9-fix（2026-05-15）GAS_URL を新デプロイ（v10）に更新
...
開発中に判明した、後の保守・再設計時に必ず知っておくべき注意点。

### 9-1. GAS 再デプロイの落とし穴

**コードの保存（Cmd+S）だけではウェブアプリに反映されない。**

毎回以下の手順が必要：

1. Apps Script エディタで Cmd+S 保存
2. 右上「デプロイ」→「デプロイを管理」
3. 既存デプロイの**鉛筆アイコン**をクリック
4. バージョン → **「新バージョン」** を選択 ← ここを「新バージョン」にしないと反映されない
5. 「デプロイ」ボタンをクリック
6. URLが変わっていないことを確認

### 9-2. GAS の謎挙動（既存デプロイ更新が時々反映されない）

**現象**: 既存デプロイの更新を「新バージョン」で正しく行っても、時々ウェブアプリに反映されない。Phase 9（バージョン9）で発生。

**対処**: 「新しいデプロイ」で完全に新規ウェブアプリを作成する。

- 新URL になるので、index.html の `GAS_URL` 定数も更新が必要
- Phase 9-fix でバージョン10として新規デプロイし、URL を変更済み

### 9-3. Google Sheets の Date 型自動変換

**現象**: `"09:00"` のような HH:MM 文字列をシートに書き込むと、Google Sheets が内部的に Date 型に自動変換する。

**影響**: doGet で取得すると `"Fri Dec 30 1899 09:00:00 GMT+0900"` のような文字列になり、フロントの正規表現で弾かれて時刻表示が消える。

**対処（Phase 9 で実装済み）**: doGet で `row[2] instanceof Date` をチェックして `Utilities.formatDate(row[2], tz, 'HH:mm')` で変換。

### 9-4. CORS preflight 対応

**現象**: Content-Type: application/json で POST すると、ブラウザが preflight OPTIONS リクエストを送信し、GAS が 405 で拒否する。

**対処（Phase 8 で実装済み）**: Content-Type: text/plain;charset=utf-8 に変更。GAS 側は `JSON.parse(body)` するので問題なし。
